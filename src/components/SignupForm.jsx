...
const SubmitButton = props => (
  ...
  <button type="submit" className="button" disabled={props.isDisabled}>
    Get Started
  </button>
);
...
export default class SignupForm extends Component {
  ...
  handleSubmit = event => {
    ...
    fetch(`${hostnameContext()}signup?email=${this.state.email}`, {
      ...
      .then(() => {
        window.location.href = `${hostnameContext()}signup?email=${
          this.state.email
        }`;
      })
      .catch(() => {
      ...
      <div className={styles.signup}>
        ...
        <div className={`${styles.signUpForm}`}>
          ...
          >
            ...
            <SubmitButton isDisabled={this.state.isDisabled} />
          </form>
        </div>
        ...
        <p className={`${'text-center'} ${styles.smallText}`}>
          <small>
            Start building with OpenLaw for{' '}
            <a href={`${hostnameContext()}signup`}>free</a>.
          </small>
          <small>
            Questions? Contact our{' '}
            <a href="mailto:hello@openlaw.io">sales team</a>.
          </small>
        </p>
      </div>
    );
